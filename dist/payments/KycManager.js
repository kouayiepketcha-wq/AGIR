...
class KycManager {
     ...
     */
    static async submitKyc(userId, documentUrl) {
        try {
            // 1. Update profile KYC state
            const { error: pError } = await supabaseClient_1.supabase
                .from('user_profiles')
                .update({
                kyc_status: 'pending',
            ...
            const { error: aError } = await supabaseClient_1.supabase
     ...
     */
    static async validateTransactionLimits(userId, amountXaf) {
        try {
            // Call the secure limits validator RPC function
            const { data, error } = await supabaseClient_1.supabase.rpc('check_user_transaction_limits', {
                p_user_id: userId,
     ...
     */
    static async getKycStats(userId) {
        try {
            ...
            const { data: dailyTx, error: dError } = await supabaseClient_1.supabase
                .from('transactions')
                .select('amount_xaf')
                .eq('user_id', userId)
            ...
            if (dError)
                throw dError;
            const { data: monthlyTx, error: mError } = await supabaseClient_1.supabase
                .from('transactions')
                .select('amount_xaf')
                .eq('user_id', userId)
                .in('type', ['deposit', 'withdrawal'])
