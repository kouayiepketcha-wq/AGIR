...
export class TransactionManager {
   ...
   */
  private static getProvider(name: string): PaymentProvider {
  ...
  ): Promise<{ success: boolean; transactionId?: string; paymentUrl?: string; ussdCode?: string; error?: string }> {
    try {
      ...
      const limitsCheck = await KycManager.validateTransactionLimits(userId, amountXaf);
  ...
  ): Promise<{ success: boolean; transactionId?: string; require2FA?: boolean; error?: string }> {
    try {
   ...
   */
  static async runReconciliationJob(): Promise<{ reconciledCount: number; expiredCount: number }> {
    ...
    try {
      ...
      for (const tx of pendingTxs) {
        ...
        if (providerName === 'BankTransfer') continue;
        ...
        const actualStatus: TransactionStatus = await provider.checkStatus(tx.id);
