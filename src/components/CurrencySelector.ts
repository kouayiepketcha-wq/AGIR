...
export class CurrencySelector {
  ...
  private onSelectCallback: (rate: CurrencyRate) => void;
  ...
  public async loadRates(): Promise<void> {
    try {
      ...
      if (data) {
        this.currencyRates = data as CurrencyRate[];
      }
    } catch (err) {
  ...
  public getRateForSelected(): number {
  ...
  private render(): void {
    ...
    const dropdown = document.getElementById('currency-dropdown') as HTMLSelectElement;
    if (dropdown) {
