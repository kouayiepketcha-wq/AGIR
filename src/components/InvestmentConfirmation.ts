...
export class InvestmentConfirmation {
  ...
  private assetPriceXaf: number;
  
  private selectedCurrency: string = 'XAF';
  private currentRate: number = 1.0;
  
  private onCompleteCallback: (result: FXConversionResult) => void;
  ...
  constructor(
    ...
    quantity: number, 
    assetName: string, 
    assetPriceXaf: number,
    currencyCode: string,
    fxRate: number,
    onComplete: (result: FXConversionResult) => void,
    onCancel: () => void
  ) {
  ...
  public render(): void {
    ...
    if (cancelBtn) cancelBtn.addEventListener('click', () => this.onCancelCallback());

    const closeBtn = document.getElementById('btn-cancel-confirmation');
    if (closeBtn) closeBtn.addEventListener('click', () => this.onCancelCallback());
  ...
  private async executeInvestment(): Promise<void> {
    const pinField = document.getElementById('confirm-pin-field') as HTMLInputElement;
    ...
    const confirmBtn = document.getElementById('btn-confirm-investment-action') as HTMLButtonElement;
    if (confirmBtn) {
    ...
    try {
      ...
      const result = data as FXConversionResult;
